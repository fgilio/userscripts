...
| `github-auto-expand-single-check-group` | `github.com/*/*/pull/*` | On the PR Checks tab, expands the only check group when there is just one |
...
notice; when a script goes quiet, check the console first — each one names the selector
that missed.
...
```
For each script in the zip  (Tampermonkey 5.5.0, background.js -> Kc)
...
Nothing outside the zip is touched, so deleting first only risks losing `GM_setValue`
data — and buys nothing. The 8 exported scripts carry their original `options.json`, so
their uuid, enabled state and sidebar position all survive. `github-actions-ci-branch-pin`
has no export yet and is matched by `@name` + `@namespace`, so it installs cleanly whether
or not it is already there.

Two consequences worth knowing:

- **`@name` and `@namespace` are the fallback identity.** This is why they must never change
...
Retired scripts are excluded from the zip. Importing will not remove them — delete those in
...
Not yet initialised — `.gitignore` is ready, so `git init && git add . && git commit` is all
it takes. Keep the remote private if `backup/` is ever un-ignored.
