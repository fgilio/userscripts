...
| [`github-notifications-mark-all-done`](https://raw.githubusercontent.com/fgilio/userscripts/main/scripts/github-notifications-mark-all-done.user.js) | `github.com/notifications*` | In the inbox grouped by repository, adds **Mark all N as done** beside a group's **Mark as done** when the group shows fewer notifications than it holds. The native button only marks the rows on screen. This one submits GitHub's own "select all matching" form from the group's *View all* page, so the server marks the whole set in one request |
