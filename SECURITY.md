...
| `github-notifications-mark-all-done` | On click only: one same-origin `fetch` of the group's "View all" notifications page, with `credentials: 'same-origin'`, then one native form POST to `/notifications/beta/archive` | none | Marks notifications as done on your behalf, only when you click its button. Posts exactly the fields of GitHub's own select-all form on that page (its token, the group's `repo:` query, `mark_all=1`), and refuses any form whose query differs from the group's |
