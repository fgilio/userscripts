...
| `github-notifications-mark-all-done` | On the confirming click only, all same-origin to `github.com` with `credentials: 'same-origin'`: one `fetch` of the group's "View all" page, one `fetch` POST to `/notifications/beta/archive` (as GitHub's own bulk-action JS sends it), then up to 5 re-reads of the "View all" page while the queued mark lands | none | Marks notifications as done on your behalf, only after two clicks on its button. Posts exactly three fields taken from GitHub's own select-all form on that page (its token, the group's `repo:` query, `mark_all=1`), and refuses any form whose query differs from the group's or drops a filter of the inbox on screen |
