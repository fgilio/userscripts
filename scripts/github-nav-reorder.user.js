...
(function () {
  ...
  function injectStyle() {
    const ID = 'fg-nav-reorder-styles';
...
${NAV} ${VIS_SEL} > li:not([data-fg-overflow-container]) { flex: 0 0 auto !important; }
${NAV} li[data-fg-overflow-container] { display: none !important; }
...
/* Hidden until reordered, so the native order never paints. armFailsafe() reveals
   it regardless. */
${NAV} ${VIS_SEL}:not([data-fg-ready]) { visibility: hidden !important; }
`;
  ...
  function buildReleasesItem(nav, repoBase) {
    ...
    li.dataset.fgNavInjected = 'releases';
    return li;
  }
  ...
  function flattenAndDedupe(nav, visible) {
    ...
    if (dd) {
      ...
      if (containerLi?.parentElement === visible) {
        containerLi.dataset.fgOverflowContainer = '1';
      }
    }
    const seen = new Set();
